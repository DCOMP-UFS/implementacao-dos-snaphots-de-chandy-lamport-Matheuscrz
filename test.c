...
typedef struct {
    ...
    int inputQueueSnapshot[QUEUE_SIZE]; // Estado da fila de entrada
    int inputQueueCountSnapshot; // Contagem da fila de entrada
    int outputQueueSnapshot[QUEUE_SIZE]; // Estado da fila de saída
    int outputQueueCountSnapshot; // Contagem da fila de saída
} Clock;
...
void Snapshot(Clock *clock) {
    ...
    // Salva o estado da fila de entrada
    pthread_mutex_lock(&inputQueue.mutex);
    for (int i = 0; i < inputQueue.count; i++) {
        clock->inputQueueSnapshot[i] = inputQueue.queue[i].p[clock->pid];
    }
    clock->inputQueueCountSnapshot = inputQueue.count;
    pthread_mutex_unlock(&inputQueue.mutex);

    // Salva o estado da fila de saída
    pthread_mutex_lock(&outputQueue.mutex);
    for (int i = 0; i < outputQueue.count; i++) {
        clock->outputQueueSnapshot[i] = outputQueue.queue[i].p[clock->pid];
    }
    clock->outputQueueCountSnapshot = outputQueue.count;
